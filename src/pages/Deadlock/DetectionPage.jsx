...
import DetectDeadlock from "../../algo/Deadlock/Detection";
...
export default function DetectionPage() {
  ...
  //Parsed
  const { available, max, alloc, request } = useResourceParser({
    availableTxt,
    maxTxt,
    allocTxt,
    requestTxt,
  ...
  const runDetect = () => {
    console.log("Deadlock Detection Algorithm is running....");
    const { deadlocked, stuck, steps } = DetectDeadlock(
      alloc,
      request,
      available
    );
    setSafe(!deadlocked);
    setStart(true);
    setSafeSeq(stuck); // reuse to show stuck set
    setNeed([]);
    setSteps(steps);
    setIdx(0);
    setAuto(true);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white py-8 px-4 flex flex-col items-center">
      <Header algo="Deadlock Detection Algorithm" />
